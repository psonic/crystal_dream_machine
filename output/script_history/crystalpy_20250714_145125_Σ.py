...
from components.svg_parser import extract_contours_from_svg, extract_and_save_contours_from_pdf, create_unified_mask
...
def main():
    ...
    try:
        ...
        # Il percorso del PDF sorgente e dell'SVG di output
        pdf_source_path = "input/CT-Logo.pdf"
        svg_output_path = "input/logo-svg-extracted-final.svg"

        # Estrai sempre i contorni dal PDF per la massima precisione
        print(f"{C_CYAN}🔬 Inizio estrazione contorni da '{pdf_source_path}'...{C_END}")
        extract_and_save_contours_from_pdf(
            pdf_path=pdf_source_path, 
            svg_output_path=svg_output_path
        )
        
        # Ora usa l'SVG appena generato come unica fonte
        print(f"{C_CYAN}🎨 Caricamento contorni dal nuovo SVG: '{svg_output_path}'...{C_END}")
        contours, hierarchy = extract_contours_from_svg(svg_output_path, Config.WIDTH, Config.HEIGHT, Config.LOGO_PADDING)

        if not contours:
            print(f"{C_BOLD}{C_YELLOW}Errore critico: nessun contorno valido trovato dall'SVG generato. Uscita.{C_END}")
            return
