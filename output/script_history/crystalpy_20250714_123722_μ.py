...
def render_frame_simple(contours, hierarchy, width, height, frame_index, total_frames, config, bg_frame, texture_image, tracer_history, bg_tracer_history, lenses):
    """
    Rendering SEMPLICE: solo SVG puro su sfondo nero, nessun effetto.
    """
    # Sfondo nero
    final_frame = np.zeros((height, width, 3), dtype=np.uint8)
    
    # Crea maschera SVG senza deformazioni
    logo_mask = np.zeros((height, width), dtype=np.uint8)
    
    # Disegna i contorni SVG esattamente come sono
    cv2.fillPoly(logo_mask, contours, 255)
    
    # Applica il colore del logo
    logo_layer = np.zeros_like(final_frame)
    logo_layer[logo_mask > 0] = config.LOGO_COLOR
    
    # Combina sfondo nero + logo
    final_frame[logo_mask > 0] = logo_layer[logo_mask > 0]
    
    return final_frame, None, None
    """
...
def main():
    ...
    try:
        ...
        for i in range(Config.TOTAL_FRAMES):
            ...
            frame_result = render_frame_simple(contours, hierarchy, Config.WIDTH, Config.HEIGHT, i, Config.TOTAL_FRAMES, Config, bg_frame, texture_image, tracer_history, bg_tracer_history, lenses)
